...
public class EncodingManagerImpl extends EncodingManager
    implements PersistentStateComponent<EncodingManagerImpl.State>, Disposable {

    private static final Logger LOG = Logger.getInstance(com.intellij.openapi.vfs.encoding.EncodingManagerImpl.class);
    ...
    private final ExecutorService changedDocumentExecutor = AppExecutorUtil.createBoundedApplicationPoolExecutor(
        "EncodingManagerImpl Document Pool", 1);
    ...
    private static boolean isEditorOpenedFor(Document document) {
        ...
        return project != null && !project.isDisposed() && FileEditorManager.getInstance(project).getEditors(
    ...
    private void handleDocument(@NotNull final Document document) {
        ...
        try {
            ...
            Charset charset = LoadTextUtil.charsetFromContentOrNull(project, virtualFile,
    ...
    public void clearDocumentQueue() {
        if (ApplicationManager.getApplication().isWriteAccessAllowed()) {
            throw new IllegalStateException(
                "Must not call clearDocumentQueue() from under write action because some queued detectors require "
                    + "read action");
        }
    ...
    public Charset getDefaultCharset() {
        return myState.myDefaultEncoding == ChooseFileEncodingAction.NO_ENCODING ? CharsetToolkit
    ...
    @Override
    public void addPropertyChangeListener(@NotNull final PropertyChangeListener listener, @NotNull
    ...
    void firePropertyChange(@Nullable Document document, @NotNull String propertyName, final Object oldValue,
